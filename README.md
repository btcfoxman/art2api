...
管理后台任务详情和管理查询的 `preparation_timing` 展示素材处理、网页验证、报价校验、额度检查/建会话及准备总耗时；验证另外记录排队、页面准备、SDK 耗时及进程/页面是否复用。准备总耗时不包含最后的生成提交请求和上游生成，历史任务没有该字段。`web_verification` 事件关联任务并记录进程 PID，便于核验真实复用。

## 对外错误文案

`/v1/*` 与 `/api/v3/*` 的错误消息只能从 [固定文案白名单](app/public_errors.py) 中选择，逐字返回，不拼接渠道名称、上游错误码、签名地址或诊断信息。生成失败、历史任务查询、幂等重放、参数/鉴权错误、404/405 和未预期异常统一遵循该规则。

| 内部原因 | 对外文案 |
| --- | --- |
| 上游明确报告维护状态 | 上游维护中，请稍后再试~ |
| 排队/并发/额度不足 | 上游队列排队限额，请稍后再试~ |
| 代理/服务异常、提交结果未知 | 队列排队服务中断，请稍后再试~ |
| 参考视频审核或版权拒绝 | 检测到视频有敏感或违规内容，请修改后重试，积分已返还～ |
| 参考音频或其他内容审核拒绝 | 检测到内容有敏感或违规情况，请修改后重试，积分已返还～ |
| 素材时长、数量、格式、下载等问题 | 从白名单选择对应的素材提示 |
| 其他明确生成失败 | 生成失败，积分已返还，请重试~ |

HTTP 状态及通用机器错误码用于程序判断；`entitlement_unavailable`、`idempotency_conflict`、`submission_unknown`、`upstream_outcome_unknown` 保持原有路由和幂等语义。未知提交结果使用队列服务中断文案，继续停止自动重提。原始错误、上游错误码、素材/提示词处理记录及准备期耗时仅在管理接口与后台详情中展示。历史任务在序列化时套用白名单，不改写原始诊断记录。
...
上游 `PROVIDER_CONTENT_SAFETY_VIOLATION` 在管理后台明确显示内容审核失败；若上游原因明确为输出音频或输出视频版权限制，控制台显示相应原因。`FILE_OPTIMIZATION_FAILED` 若注明输入图/视/音的版权限制或敏感内容，后台也会明确指出被拒的素材类型；渠道查询仅返回上述白名单文案。内部保留结构化上游错误码，不暴露原始响应中的签名 URL。此类任务保持失败，不在 art2api 内自动重新生成；追加静音只解决音频时长不足，不改变审核结果。
