...
- **授权浏览器**：闲置超时、无头模式。无头模式对下次启动的浏览器生效，默认关闭。
...
Seedance 2.5 会出现长提示词末尾的音频引用在上游映射失败的情况；适配器在开头列出原文已经引用的音频标记，保留完整提示词及素材序号，不补写未引用素材的标签。报价与生成使用相同文本，原始请求保留，任务详情展示这项兼容处理。
