...
上游 `PROVIDER_CONTENT_SAFETY_VIOLATION` 会明确显示内容审核失败；若上游原因明确为输出音频或输出视频版权限制，控制台及渠道查询会显示相应原因。`FILE_OPTIMIZATION_FAILED` 若注明输入图/视/音的版权限制或敏感内容，也会明确指出被拒的素材类型。保留结构化上游错误码，不暴露原始响应中的签名 URL。此类任务保持失败，不在 art2api 内自动重新生成；追加静音只解决音频时长不足，不改变审核结果。
