...
Seedance 2.0/2.5 均出现过提示词引用在上游映射失败的情况。适配器在开头用空格分隔列出原文实际引用的图/视/音标签，音频仍置于最前；映射时长编号优先，如 `@img10` 先于 `@img1`，`orderForType` 保持原素材序号。保留完整提示词和全部素材，不补写未引用素材的标签，不截断原文；报价与生成使用相同处理结果，重复处理不重复追加索引。原始请求保留，任务详情展示这项兼容处理；不存在的素材引用仍在提交前明确拒绝。
