...
function openTaskDetail(task) {
  ...
  if (task.preparation_timing) {
    ...
    if (t.media_items?.length) {
      fields.push(['素材传输', `经账号固定代理，${t.media_account_parallelism ? `单任务最多 ${t.media_parallelism} 份并行，账号素材池最多 ${t.media_account_parallelism} 份` : `每账号最多同时处理 ${t.media_parallelism} 份素材`}；下列耗时可能重叠，不能相加作为准备期总耗时。`]);
      const names = {image_urls:'图片',video_urls:'视频',audio_urls:'音频',first_frame:'首帧',last_frame:'尾帧'};
      for (const item of t.media_items) fields.push([`${names[item.field] || '素材'} ${item.index}${item.pass === 'padding' ? '（补齐静音）' : ''}`, [['总计',item.total_seconds],['任务内等待',item.task_queue_seconds],['账号等待',item.queue_seconds],['下载',item.download_seconds],['解析',item.probe_seconds],['转换',item.transform_seconds],['上传签名',item.sign_upload_seconds],['上传',item.upload_seconds],['下载签名',item.sign_read_seconds],['可读检查',item.read_check_seconds]].filter(([,n]) => Number.isFinite(n)).map(([name,n]) => `${name} ${seconds(n)}`).join('；')]);
    }
    if (t.requests) {
